...
namespace reflow {
	...
	public:
		basic_reader_traits(std::string);
...
template <typename ReadType, typename ReadMethod>
reflow::basic_reader_traits<ReadType, ReadMethod>::basic_reader_traits(std::string file_name)
	: __M_traits_string(std::move(file_name)) {  }

template <typename ReadType, typename ReadMethod>
bool reflow::basic_reader_traits<ReadType, ReadMethod>::open()
{
	return method_type::open(__M_traits_handle, __M_traits_string);
}

template <typename ReadType, typename ReadMethod>
bool reflow::basic_reader_traits<ReadType, ReadMethod>::close()
{
